...
const contentData = {
  wedding: {
    ...
    image: weddingImg,
    badgeColor:
      "border-pink-500/30 text-pink-600 bg-white/80 shadow-pink-200/50",
  },
  birth: {
    tabTitle: "Birth",
    ...
    image: birthImg,
    badgeColor:
      "border-purple-500/30 text-purple-600 bg-white/80 shadow-purple-200/50",
  },
  death: {
    tabTitle: "Death",
    ...
    image: deathImg,
    badgeColor:
      "border-amber-500/30 text-amber-700 bg-white/80 shadow-amber-200/50",
  },
  MobileStand: {
    tabTitle: "Mobile Stand",
    quote:
      "“আপনার নিত্যদিনের ব্যবহারের ফোনটি থাকুক একটি প্রিমিয়াম কাঠে খোদাই করা স্ট্যান্ডে।”",
    image: Stand, // Custom assets import name change thakle apply kore niben
    badgeColor:
      "border-indigo-500/30 text-indigo-700 bg-white/80 shadow-indigo-200/50",
  },
  TesuBox: {
    tabTitle: "Tissue Box",
    quote:
      "“প্রিমিয়াম ডিজাইনের টিস্যু বক্স যা আপনার ড্রয়িং রুম বা ডেস্কে আনবেভি আভিজাত্যের ছোঁয়া।”",
    image: TesuBox, // Custom assets import name change thakle apply kore niben
    badgeColor:
      "border-emerald-500/30 text-emerald-700 bg-white/80 shadow-emerald-200/50",
  },
  ChabiRing: {
    tabTitle: "Keychain",
    quote:
      "“আপনার সবচেয়ে প্রিয় চাবিগুলো সুরক্ষিত থাকুক আপনার নিজের নামের খোদাই করা রিং-এ।”",
    image: deathImg,
    badgeColor:
      "border-rose-500/30 text-rose-700 bg-white/80 shadow-rose-200/50",
  },
};
...
const HeroPage = ({ selectedTab, setSelectedTab, setIsModalOpen }) => {
    ...
    const incrementViewCount = async () => {
      try {
        const response = await fetch(
          "https://pixelwood-server.vercel.app/views/increment",
          {
            method: "POST",
            cache: "no-store",
            headers: {
        ...
        if (!response.ok) {
          ...
          console.error(
            `Backend Returned HTML/Error instead of JSON (${response.status}):`,
            errorHtmlText,
          );
          return;
        }
  ...
  const tabs = [
    ...
    { key: "MobileStand", title: "Stand", icon: Stand },
  ...
  return (
    <div className="text-slate-800 p-4 md:py-16 md:px-20 font-sans flex flex-col items-center justify-center min-h-screen selection:bg-purple-200">
      ...
      <div className="text-center mb-10 z-10">
        <p className="text-[11px] tracking-[0.5em] text-white uppercase font-black drop-shadow-sm opacity-90">
          PIXELWOOD PREMIUM
        </p>
        <h1 className="text-4xl md:text-6xl font-serif mt-2 text-white tracking-tight drop-shadow-[0_10px_20px_rgba(0,0,0,0.2)] font-bold">
          স্মৃতি চিরন্তন
        </h1>
      ...
      >
        ...
        <div className="relative p-8 md:p-12 bg-gradient-to-br from-white/40 to-white/10 flex flex-col items-center justify-center min-h-[440px] md:min-h-[520px] overflow-hidden border-b md:border-b-0 md:border-r border-white/40">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="relative w-full max-w-[320px] md:max-w-[550px] group/img perspective-1000">
                ...
                <div className="relative bg-white p-3 rounded-2xl border z-10 border-white shadow-[0_20px_40px_rgba(0,0,0,0.15)] transition-all duration-500 transform group-hover/img:-rotate-2 group-hover/img:-translate-y-2 group-hover/img:shadow-[0_30px_50px_rgba(0,0,0,0.25)]">
                  <Image
                    src={data.image}
                    alt="Product Preview"
                    priority
          ...
          <div className="absolute bottom-6 left-6 z-10">
            <div
              className={`flex items-center gap-2 border px-4 py-2 rounded-full text-xs font-black uppercase tracking-wider shadow-[0_8px_20px_rgba(0,0,0,0.06)] border-white/60 ${data.badgeColor}`}
            >
              <span>
                <FaRegStar className="animate-pulse text-xs" />
              </span>{" "}
              {data.tabTitle}
            </div>
          </div>
        ...
        <div className="p-8 md:p-12 flex flex-col justify-center bg-white/30 backdrop-blur-2xl relative">
          <div className="mb-8">
            <h2 className="text-3xl md:text-4xl font-black text-slate-900 mb-3 tracking-tight drop-shadow-sm">
              আপনার বিশেষ দিনটি খোদাই করুন
            </h2>
            <p className="text-slate-700 text-xs md:text-sm leading-relaxed font-medium opacity-90">
              প্রিমিয়াম লেজার-এনগ্রেভড কার্ড এবং কাঠের প্রোডাক্টের উপর আপনার
              জীবনের সেরা মুহূর্তগুলো আজীবন সংরক্ষণ করুন।
            </p>
          </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {tabs.map((tab) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-10 h-10 flex items-center justify-center rounded-full border mb-1.5 overflow-hidden transition-all duration-300 ${
                        isActive
                    ...
                    <span
                      className={`text-[9px] font-black uppercase z-50 tracking-wider text-center line-clamp-1 ${
          ...
          <div className="flex justify-between items-center p-4 bg-white/80 rounded-2xl border border-white mb-8 shadow-[0_8px_25px_rgba(0,0,0,0.04)]">
            ...
            <div className="text-right">
              <span className="text-3xl font-black text-purple-700 tracking-tight drop-shadow-sm">
                999.00 BDT
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              className="flex items-center justify-center gap-2 bg-white hover:bg-slate-50 text-slate-700 py-3 rounded-xl text-xs font-bold border border-slate-200/80 hover:border-slate-300 transition-all duration-300 shadow-sm active:scale-95"
            >
              <LuMessageSquareMore className="text-base text-[#0084FF]" />{" "}
              Messenger
            </button>
            <button
              type="button"
              className="flex items-center justify-center gap-2 bg-white hover:bg-emerald-50 text-emerald-700 py-3 rounded-xl text-xs font-bold border border-emerald-200/60 hover:border-emerald-300 transition-all duration-300 shadow-sm active:scale-95"
            >
